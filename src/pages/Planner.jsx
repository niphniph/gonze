...
export const Planner = ({ setActivePage }) => {
  ...
  return (
    <div className="planner-page">
      <header className="page-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2.5rem', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          <h1 className="text-gradient" style={{ fontSize: '2.25rem', fontWeight: 800 }}>დღიური პლანერი</h1>
          <p style={{ color: 'hsl(var(--text-secondary))', marginTop: '0.25rem' }}>სრული კონტროლი თქვენს დღევანდელ გეგმებზე, დავალებებსა და ჩვევებზე</p>
        </div>
      ...
      <section style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1.5rem', marginBottom: '2.5rem' }}>
        <GlassCard style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
          <span style={{ fontSize: '0.75rem', fontWeight: 700, textTransform: 'uppercase', color: 'hsl(var(--text-muted))' }}>დავალებების პროგრესი</span>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
            <h3 style={{ fontSize: '1.8rem', fontWeight: 800 }}>{taskProgress}%</h3>
            <span style={{ fontSize: '0.85rem', color: 'hsl(var(--text-secondary))' }}>
              {completedTodayTasks.length}/{todayTasks.length} შესრულდა
            </span>
          </div>
        ...
        <GlassCard style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
          <span style={{ fontSize: '0.75rem', fontWeight: 700, textTransform: 'uppercase', color: 'hsl(var(--text-muted))' }}>ჩვევების შესრულება</span>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
            <h3 style={{ fontSize: '1.8rem', fontWeight: 800 }}>{habitProgress}%</h3>
            <span style={{ fontSize: '0.85rem', color: 'hsl(var(--text-secondary))' }}>
              {completedTodayHabits.length}/{habits.length} შესრულდა
            </span>
          </div>
        ...
        <GlassCard style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '1rem' }}>
          <div>
            <span style={{ fontSize: '0.75rem', fontWeight: 700, textTransform: 'uppercase', color: 'hsl(var(--text-muted))' }}>დღევანდელი შეხვედრები</span>
            <h3 style={{ fontSize: '1.8rem', fontWeight: 800, marginTop: '0.25rem' }}>{todayMeetings.length} ივენთი</h3>
          </div>
          <button 
          ...
          <GlassCard style={{ padding: '1.25rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem' }}>
              <h3 style={{ fontSize: '1.1rem', fontWeight: 700, display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                ...
                დღევანდელი დავალებები ({todayTasks.length})
              </h3>
              <button 
          ...
          <GlassCard style={{ padding: '1.25rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem' }}>
              <h3 style={{ fontSize: '1.1rem', fontWeight: 700, display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                ...
                ჩვევების შემოწმება ({habits.length})
              </h3>
              <button 
        ...
        <GlassCard style={{ display: 'flex', flexDirection: 'column' }}>
          ...
          {todayMeetings.length > 0 ? (
              ...
              {todayMeetings.map((meeting, index) => (
                ...
                >
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.25rem' }}>
                    ...
                    <span style={{ fontSize: '0.75rem', color: 'hsl(var(--accent-blue))', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '0.2rem' }}>
                      <Clock size={12} />
                      {meeting.time}
                    </span>
                  </div>
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '1rem', color: 'hsl(var(--text-muted))', padding: '4rem 1rem' }}>
              ...
              <span style={{ fontSize: '0.85rem' }}>დღეს შეხვედრები არ გაქვთ დაგეგმილი</span>
            </div>
          )}
        </GlassCard>
      </div>
